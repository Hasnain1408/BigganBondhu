...
export default function ProjectileMotionSimulation() {
  ...
  const animate = () => {
    ...
    ctx.fillText(
      lang === "bn" ? `ব্যাপ্তি: ${range.toFixed(1)} মি` : `Range: ${range.toFixed(1)} m`,
      width - 150,
      30
    )
    ctx.fillText(
      lang === "bn" ? `সর্বোচ্চ উচ্চতা: ${maxHeight.toFixed(1)} মি` : `Max Height: ${maxHeight.toFixed(1)} m`,
      width - 150,
      55
    )
    ctx.fillText(
      lang === "bn" ? `উড্ডয়নের সময়: ${timeOfFlight.toFixed(1)} সে` : `Flight Time: ${timeOfFlight.toFixed(1)} s`,
      width - 150,
      80
    )
  ...
  }, [isPlaying, initialVelocity, angle, gravity, timeScale, time, lang])
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card>
            <CardContent className="pt-6">
              ...
              />
              <p className="text-right mt-2">{initialVelocity.toFixed(0)} {lang === "bn" ? "মি/সে" : "m/s"}</p>
            </CardContent>
          </Card>

          <Card>
            <CardContent className="pt-6">
              <Label>{lang === "bn" ? "উৎক্ষেপণ কোণ (°)" : "Launch Angle (°)"}</Label>
              <Slider
                min={5}
                max={85}
                step={1}
          ...
          <Card>
            <CardContent className="pt-6">
              <Label>{lang === "bn" ? "মাধ্যাকর্ষণ (মি/সে²)" : "Gravity (m/s²)"}</Label>
              <Slider
                min={1}
                max={20}
                step={0.1}
              ...
              />
              <p className="text-right mt-2">{gravity.toFixed(1)} {lang === "bn" ? "মি/সে²" : "m/s²"}</p>
            </CardContent>
          </Card>
