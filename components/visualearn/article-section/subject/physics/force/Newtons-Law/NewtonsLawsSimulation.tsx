...
export default function NewtonsLawsSimulation() {
  ...
  const calculateMotion = () => {
    const acceleration = force / mass // Newton's Second Law: a = F/m
  ...
  const animate = () => {
    ...
    const { displacement, velocity, acceleration } = calculateMotion()
    ...
    ctx.fillText(
      lang === "bn" ? `ত্বরণ: ${acceleration.toFixed(1)} মি/সে²` : `Acceleration: ${acceleration.toFixed(1)} m/s²`,
      width - 150,
      80
    )
    ctx.fillText(
  ...
  }, [isPlaying, force, mass, timeScale, time, lang])
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card>
            <CardContent className="pt-6">
              <Label>{lang === "bn" ? "বল (নিউটন)" : "Force (N)"}</Label>
              <Slider
                min={0}
                max={20}
                step={0.1}
              ...
              />
              <p className="text-right mt-2">{force.toFixed(1)} {lang === "bn" ? "নি" : "N"}</p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="pt-6">
              ...
              />
              <p className="text-right mt-2">{mass.toFixed(1)} {lang === "bn" ? "কেজি" : "kg"}</p>
            </CardContent>
          </Card>
