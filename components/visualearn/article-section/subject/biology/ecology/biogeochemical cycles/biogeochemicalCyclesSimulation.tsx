"use client"

import { useEffect, useRef } from "react"
import p5 from "p5"
import { Slider } from "@/components/ui/slider"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"

export default function Simulation() {
  const sketchRef = useRef()
  let plants = []
  let animals = []
  let co2 = []
  let CO2Level = 50
  let tooltip = ""

  const sketch = (p) => {
    p.setup = () => {
      p.createCanvas(800, 600)
      for (let i = 0; i < 10; i++) {
        plants.push({ x: p.random(p.width), y: p.random(p.height) })
      }
      for (let i = 0; i < 5; i++) {
        animals.push({ x: p.random(p.width), y: p.random(p.height) })
      }
      for (let i = 0; i < CO2Level / 2; i++) {
        co2.push({ x: p.random(p.width), y: p.random(p.height) })
      }
    }

    p.draw = () => {
      p.background(173, 216, 230)
      plants.forEach(pl => {
        p.fill(0, 128, 0)
        p.rect(pl.x - 10, pl.y - 20, 20, 40)
      })

      animals.forEach(a => {
        p.fill(139, 69, 19)
        p.ellipse(a.x, a.y, 20)
        a.x += p.random(-2, 2)
        a.y += p.random(-2, 2)
        a.x = p.constrain(a.x, 0, p.width)
        a.y = p.constrain(a.y, 0, p.height)
        if (Math.random() < 0.01) co2.push({ x: a.x, y: a.y })
      })

      co2.forEach((c, i) => {
        p.fill(135, 206, 255)
        p.ellipse(c.x, c.y, 10)
        c.x += p.random(-1, 1)
        c.y += p.random(-1, 1)
        c.x = p.constrain(c.x, 0, p.width)
        c.y = p.constrain(c.y, 0, p.height)

        plants.forEach(pl => {
          if (p.dist(c.x, c.y, pl.x, pl.y) < 20 && Math.random() < 0.01) {
            co2.splice(i, 1)
          }
        })
      })

      if (p.mouseX > 0 && p.mouseX < p.width && p.mouseY > 0 && p.mouseY < p.height) {
        let closest = findClosest(p.mouseX, p.mouseY)
        if (closest) tooltip = closest.type
      }

      if (tooltip) {
        p.fill(0, 0, 0, 200)
        p.rect(p.mouseX + 10, p.mouseY - 20, 80, 30)
        p.fill(255)
        p.text(tooltip, p.mouseX + 15, p.mouseY - 5)
      }
    }

    const findClosest = (mx, my) => {
      let minDist = Infinity
      let closest = null
      plants.forEach(p => {
        let d = p.dist(mx, my, p.x, p.y)
        if (d < minDist && d < 20) {
          minDist = d
          closest = { type: "Plant" }
        }
      })
      animals.forEach(a => {
        let d = p.dist(mx, my, a.x, a.y)
        if (d < minDist && d < 20) {
          minDist = d
          closest = { type: "Animal" }
        }
      })
      co2.forEach(c => {
        let d = p.dist(mx, my, c.x, c.y)
        if (d < minDist && d < 10) {
          minDist = d
          closest = { type: "CO2" }
        }
      })
      return closest
    }
  }

  useEffect(() => {
    const p5Instance = new p5(sketch, sketchRef.current)
    return () => p5Instance.remove()
  }, [])

  const resetSimulation = () => {
    plants = []
    animals = []
    co2 = []
    for (let i = 0; i < 10; i++) {
      plants.push({ x: Math.random() * 800, y: Math.random() * 600 })
    }
    for (let i = 0; i < 5; i++) {
      animals.push({ x: Math.random() * 800, y: Math.random() * 600 })
    }
    for (let i = 0; i < CO2Level / 2; i++) {
      co2.push({ x: Math.random() * 800, y: Math.random() * 600 })
    }
  }

  return (
    <div className="max-w-4xl mx-auto p-6">
      <Card className="shadow-2xl border-0 bg-white/80 backdrop-blur-sm">
        <CardHeader className="text-center bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-t-lg">
          <CardTitle className="text-3xl font-bold">Biogeochemical Cycles Simulation</CardTitle>
        </CardHeader>
        <CardContent className="p-8">
          <div className="flex flex-col items-center">
            <div ref={sketchRef} className="mb-4"></div>
            <div className="w-full max-w-md space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700">CO2 Level</label>
                <Slider
                  min={0}
                  max={100}
                  step={1}
                  value={[CO2Level]}
                  onValueChange={([val]) => {
                    CO2Level = val
                    co2 = []
                    for (let i = 0; i < CO2Level / 2; i++) {
                      co2.push({ x: Math.random() * 800, y: Math.random() * 600 })
                    }
                  }}
                  className="mt-2"
                />
                <p className="text-sm text-gray-500">Current: {CO2Level}%</p>
              </div>
              <Button onClick={resetSimulation} className="w-full bg-blue-600 hover:bg-blue-700">
                Reset Simulation
              </Button>
            </div>
          </div>
        </CardContent>
      </Card>
    </div>
  )
}