...
export default function PeriodicTrendsSimulation() {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const requestRef = useRef<number | null>(null)
  const [isPlaying, setIsPlaying] = useState(false)
  const [trendIndex, setTrendIndex] = useState(0) // 0: Atomic Radius, 1: Ionization Energy, 2: Electronegativity
  ...
  const trends = [
    { name: "Atomic Radius", unit: "pm", key: "radius", max: 152 },
    { name: "Ionization Energy", unit: "kJ/mol", key: "ionization", max: 1681 },
    { name: "Electronegativity", unit: "", key: "electronegativity", max: 3.98 },
  ]
  ...
  const drawTrend = (ctx: CanvasRenderingContext2D) => {
    ...
    // Draw periodic table layout (simplified period 2)
    elements.forEach((element, i) => {
      const x = 100 + i * 100
      const y = canvasHeight / 2
      const value = element[trend.key as keyof typeof element] as number
      const scale = value / trend.max * 50
      ...
      // Draw value label
      if (showLabels) {
        ctx.fillText(`${value}${trend.unit}`, x, y - 60)
    ...
    ctx.textAlign = "left"
    ctx.fillText(
      lang === "bn" ? `প্রবণতা: ${trend.name}` : `Trend: ${trend.name}`,
      20,
      35
    )
  }
  ...
  }, [isPlaying, trendIndex, showLabels, animationSpeed, animationTime, lang])
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card>
            <CardContent className="pt-4">
              <Label className="text-sm">{lang === "bn" ? "প্রবণতা নির্বাচন" : "Select Trend"}</Label>
              <Slider
                min={0}
                max={2}
                step={1}
                value={[trendIndex]}
                onValueChange={(v) => setTrendIndex(v[0])}
                className="mt-2"
              />
              <p className="text-right mt-1 text-sm">{trends[trendIndex].name}</p>
            </CardContent>
          </Card>
